...
from airflow.sdk import dag, task_group, task, chain, chain_linear, Label, Asset
...
def complex_dag_structure():
    ...
    @task
    def internal_api_load_full():
        import logging
        import time

        log = logging.getLogger(__name__)

        log.info("Starting full load from internal API")
        log.info("Authenticating with internal API using service account credentials")
        log.info("Authentication successful — token expires in 3600s")
        log.info("Fetching paginated records from /api/v2/sales/full-export (page_size=1000)")
        time.sleep(1)
        log.info("Page 1/8 received — 1000 records (elapsed: 0.31s)")
        log.info("Page 2/8 received — 1000 records (elapsed: 0.59s)")
        log.info("Page 3/8 received — 1000 records (elapsed: 0.88s)")
        log.info("Page 4/8 received — 1000 records (elapsed: 1.14s)")
        log.info("Page 5/8 received — 1000 records (elapsed: 1.43s)")
        log.info("Page 6/8 received — 1000 records (elapsed: 1.71s)")
        log.info("Page 7/8 received — 1000 records (elapsed: 1.99s)")
        log.info("Page 8/8 received — 823 records (elapsed: 2.21s)")
        log.info("Total records fetched: 7823")
        log.info("Validating schema — all fields present and types match expected contract")
        log.info("Writing 7823 records to staging table `stg_sales_full_load`")
        log.info("Full load complete — 7823 rows inserted, 0 errors")

    internal_api_load_full_obj = internal_api_load_full()
    ...
    def mlops():
        ...
        train_model = EmptyOperator(
            task_id="train_model", outlets=[Asset("model_trained")]
        )
    ...
    chain(
        ...
        [internal_api_load_full_obj, internal_api_load_incremental],
        mlops_obj,
        end,
    )

    chain_linear(
        [sales_data_load, internal_api_load_full_obj],
        [sales_data_reporting_obj, cre_integration_obj],
    )
